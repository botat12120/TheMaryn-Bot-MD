...
export async function before(m, {conn, isAdmin, isBotAdmin}) {
  ...
  if (isAdmin && chat.antiLink && m.text.includes(grupo)) return m.reply('*تم تفعيل مضاد الروابط من طرف المشرفين*');
  if (chat.antiLink && isGroupLink && !isAdmin) {
    ...
    if (!isBotAdmin) return m.reply('*البوت مامعه إشراف عان يطرد!*');
